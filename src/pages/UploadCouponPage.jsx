...
if (!profileData.upiId) {
  ...
  return;
    ...
    if (!brand.trim() || !code.trim() || !discount.trim() || !price.trim()) { 
      toast.error('Validation Error: Required fields (Brand, Code, Discount, Price) are missing.'); 
      return; 
    }
    ...
    try {
      await addDoc(collection(db, 'coupons'), {
        ...
        status: "approved", // For MVP, auto-approving. In production, this should go through an approval workflow. 
        
        // ⚡ Community Trust Foundation Fields
        workedCount: 0,
        failedCount: 0,
        ...
        <div className="lg:col-span-7 space-y-8">
          ...
          <form className="space-y-6" onSubmit={handleUploadCoupon}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="space-y-2"><label className="text-xs font-mono uppercase text-zinc-500">Brand Name</label><input type="text" required value={brand} onChange={(e) => setBrand(e.target.value)} className="w-full bg-white/[0.01] border border-white/10 rounded-xl px-4 py-3.5 text-sm text-white focus:outline-none focus:border-purple-500" /></div>
              <div className="space-y-2"><label className="text-xs font-mono uppercase text-zinc-500">Discount</label><input type="text" required value={discount} onChange={(e) => setDiscount(e.target.value)} className="w-full bg-white/[0.01] border border-white/10 rounded-xl px-4 py-3.5 text-sm text-white focus:outline-none focus:border-purple-500" /></div>
            </div>

            <div className="space-y-2"><label className="text-xs font-mono uppercase text-zinc-500">Coupon Code</label><input type="text" required value={code} onChange={(e) => setCode(e.target.value)} className="w-full bg-white/[0.01] border border-white/10 rounded-xl px-4 py-3.5 text-sm font-mono text-purple-300 focus:outline-none focus:border-purple-500" /></div>
            
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="space-y-2"><label className="text-xs font-mono uppercase text-zinc-500">Expiry Date</label><input type="date" value={expiry} onChange={(e) => setExpiry(e.target.value)} className="w-full bg-white/[0.01] border border-white/10 rounded-xl px-4 py-3.5 text-sm text-zinc-400 focus:outline-none focus:border-purple-500" /></div>
              <div className="space-y-2"><label className="text-xs font-mono uppercase text-zinc-500">Selling Price</label><input type="text" required value={price} onChange={(e) => setPrice(e.target.value)} className="w-full bg-white/[0.01] border border-white/10 rounded-xl px-4 py-3.5 text-sm text-white focus:outline-none focus:border-purple-500" /></div>
            </div>
