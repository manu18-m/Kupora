...
export default function SignupPage() {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      // Persist the role assignment directly within Firestore documents matching user uid hashes
    ...
    <div className="min-h-screen bg-[#030014] text-zinc-200 font-sans flex items-center justify-center p-4 relative overflow-hidden">
      ...
      >
        <div className="space-y-2 text-center">
          <h1 className="text-2xl font-black text-white tracking-tight">Provision Identity</h1>
          <p className="text-xs text-zinc-500 font-mono uppercase tracking-wider">Deploy New Ecosystem Node</p>
        </div>
        ...
        <form onSubmit={handleSignup} className="space-y-4">
          <div className="space-y-1.5">
            <label className="text-[11px] font-mono uppercase text-zinc-500 tracking-wider">Ecosystem Role Mapping</label>
            <div className="grid grid-cols-2 gap-2">
              {['user', 'seller'].map((r) => (
                ...
                >
                  {r === 'user' ? 'Buyer Terminal' : 'Seller Node'}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            <label className="text-[11px] font-mono uppercase text-zinc-500 tracking-wider">Email Parameter</label>
            <input 
          ...
          >
            {loading ? 'Compiling Profile Documents...' : 'Execute Node Creation'}
          </button>
        </form>
